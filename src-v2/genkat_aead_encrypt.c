...
int generate_test_vectors() {
  ...
  //unsigned char msg2[MAX_MESSAGE_LENGTH]; testo decifrato
  unsigned char ad[MAX_ASSOCIATED_DATA_LENGTH];
  unsigned char ct[MAX_MESSAGE_LENGTH + CRYPTO_ABYTES];
  unsigned char hct[MAX_MESSAGE_LENGTH + CRYPTO_ABYTES];
  unsigned long long clen, hclen, mlen2;
  int count = 1;
  ...
  sprintf(fileName, "hls_encrypt_output.txt");
  ...
  for (unsigned long long mlen = 0;
    ...
    for (unsigned long long adlen = 0; adlen <= MAX_ASSOCIATED_DATA_LENGTH;
      ...
      if ((func_ret = crypto_aead_encrypt_c(ct, &clen, msg, mlen, ad, adlen, NULL,
                                          nonce, key)) != 0) {
      ...
      if ((func_ret = crypto_aead_encrypt_hls(hct, &hclen, msg, mlen, ad, adlen, NULL,
                                          nonce, key)) != 0) {
        fprintf(fp, "crypto_aead_encrypt returned <%d>\n", func_ret);
        ret_val = KAT_CRYPTO_FAILURE;
        break;
      }
      if(clen != hclen || memcmp(ct, hct, clen)){
          fprint_bstr(stderr, "c CT: ", ct, clen);
          fprint_bstr(stderr, "h CT: ", hct, hclen);
      }
    }
  }
  fclose(fp);
